...
const Score = () => {
  ...
  return (
    <Page className={'p-0 pb-5 md:w-2/5 md:h-2/3 sm:h-4/5 overflow-scroll space-y-4'}>
      ...
      </div>
      {
        isOver ? (
          <Button onClick={() => handleClick('winner')}
                  className={'md:w-2/5 sm:w-4/5 min-h-[3rem] bg-secondary text-main'}>
            Kazananı Gör
          </Button>
        ) : (
      ...
      <Button onClick={() => handleClick('restart')} className={'md:w-2/5 sm:w-4/5 min-h-[3rem] bg-secondary text-main'}>
        Yeni Oyuna Başla
      </Button>
    </Page>
  )
